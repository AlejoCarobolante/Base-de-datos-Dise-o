...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_check_baja_carrera` BEFORE UPDATE ON `Carrera` FOR EACH ROW BEGIN
    DECLARE v_msg VARCHAR(255);

    -- Bloquear actualización redundante
    IF OLD.fechaBajaCarrera = NEW.fechaBajaCarrera THEN
        SET v_msg = 'No se puede volver a dar de baja la Carrera';
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_ValidarEstadoProyectoParaPostulacion` BEFORE INSERT ON `Postulacion` FOR EACH ROW BEGIN
    ...
    FROM Proyecto p
    JOIN ProyectoPuesto pp ON p.OIDProyecto = pp.OIDProyecto
    WHERE pp.OIDProyectoPuesto = NEW.OIDProyectoPuesto;

    -- Validar que esté en estado 2
    IF estado_proyecto <> 2 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'No se puede postular: el proyecto no está en estado 2.';
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_AutoActualizarEstadoProyecto` AFTER INSERT ON `Postulacion` FOR EACH ROW BEGIN
    DECLARE proyecto_id INT;
    DECLARE invalido INT;

    -- Obtener el proyecto asociado al puesto
    SELECT OIDProyecto INTO proyecto_id
    FROM ProyectoPuesto
    WHERE OIDProyectoPuesto = NEW.OIDProyectoPuesto;

    -- Revisar si todos los puestos cumplen rango
    SELECT COUNT(*)
    INTO invalido
    FROM ProyectoPuesto pp
    LEFT JOIN Postulacion po ON po.OIDProyectoPuesto = pp.OIDProyectoPuesto
    WHERE pp.OIDProyecto = proyecto_id
    ...
    -- Si no hay inválidos → actualizar proyecto a estado 3
    IF invalido IS NULL THEN
        UPDATE Proyecto
        SET OIDEstadoProyecto = 3
        WHERE OIDProyecto = proyecto_id;

        -- Mostrar mensaje informativo
        SIGNAL SQLSTATE '01000'
            SET MESSAGE_TEXT = 'Postulaciones cerradas por cantidad de postulaciones, proyecto pasa a estado iniciado';
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`%`*/ /*!50003 TRIGGER `trg_ValidarCambioEstadoProyecto` BEFORE UPDATE ON `Proyecto` FOR EACH ROW BEGIN
    ...
    IF NEW.OIDEstadoProyecto = 3 THEN
        ...
        -- Si existe un puesto inválido → error
        IF invalido IS NOT NULL THEN
            SIGNAL SQLSTATE '45000'
                SET MESSAGE_TEXT = 'No se puede cambiar a estado 3: algún puesto no cumple con el rango de postulaciones.';
        END IF;
    END IF;
END */;;
DELIMITER ;
...
-- Dump completed on 2025-09-19  1:41:44
